...
const editStation = () => {
  if (
    typeof toEditIndex.value === "number" &&
    typeof toEditId.value === "number"
  ) {
...
<template>
  <div style="display: flex; flex-direction: column">
            ...
            >
              <el-space>
                <el-button
                  class="change"
                  @click="
                    toEditIndex = route.station_ids.indexOf(station);
                    toEditId = station;
                    edit = true;
                  "
                >
                ...
                <div>
                  <el-icon
                    @click="deleteStation(route.station_ids.indexOf(station))"
    ...
    <el-button
      @click="$emit('formSubmitted', route)"
      ...
      <el-space>
        <el-select v-model="toEditId" filterable>
          <el-option
            v-for="item in stations.rawData"
            :disabled="
              route.station_ids.includes(item.id) && item.id !== toEditId
            "
            :key="item.id"
            :label="item.name"
            :value="item.id"
        ...
        <el-button type="primary" @click="editStation"> 确定 </el-button>
      </el-space>
    </div>
  </el-dialog>
</template>
